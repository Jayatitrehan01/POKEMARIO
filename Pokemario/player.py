...
player_starting_x, player_starting_y = 0, win_y - (lvl1_block_size + pikachu_dims[1])
    ...
    def powerup(self):
        ...
        if self.evolve:
            ...
            if self.current_time - self.start_time <= 5:
                print_text(f'{abs(5 - int(self.current_time - self.start_time))}',main_menu_font,self.rect.x,self.rect.y-25,screen)
                self.SPRITES = raichu
            else:
                self.evolve = False 
...
lvl1_bulbasaur1 = Enemy(550,win_y - (2*lvl1_block_size) + 20 - 64,100,'right',True, 3,bulbasaur)
lvl1_bulbasaur2 = Enemy(1000,win_y - (3*lvl1_block_size) + 20 - pidgeott_dims[1],150,'right',True,3,bulbasaur)
golbat1 = Enemy(500,550,500,'right',False,5,golbat)
haunter1 = Enemy(1000,550,500,'right',False,5,haunter)
